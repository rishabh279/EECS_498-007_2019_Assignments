...
def ReferenceOnActivatedAnchors(anchors, bboxes, grid, iou_mat, pos_thresh=0.7, neg_thresh=0.3, method='FasterRCNN'):
  ...
  if method == 'FasterRCNN':
    ...
    # Computes the smallest distance between center of Gt and center of gird cell for each object
    mah_dist = torch.abs(grid.view(B, -1, 2).unsqueeze(2) - bbox_centers.unsqueeze(1)).sum(dim=-1) # Bx(H'xW')xN, (B, 49, 9)
    # Gives the smallest distance b/t center for each object w.r.t all grid cells.
    ...
    # Finds the anchor with larget iou for each object
    ...
    # This gives the object which have largest IOU w.r.t anchor and which have distance minimum
    # w.r.t center of grid cell.
    ...
    # activated_anc_mask.shape -> torch.Size([3, 441, 40])
    # activated_anc_mask.view(-1).shape -> torch.Size([52920])
    # torch.nonzero(activated_anc_mask.view(-1)).shape -> torch.Size([4, 1])
    # activated_anc_ind.shape -> torch.Size([4])
    activated_anc_ind = torch.nonzero(activated_anc_mask.view(-1)).squeeze(-1) # This gives the indicies of object present in the current batch_size
    
    GT_conf_scores = iou_mat.view(-1)[activated_anc_ind] # This gives the iou score of activated anchors 
