...
def GenerateAnchor(anc, grid):
  ...
  B, H, W, _  = grid.shape
  A,_ = anc.shape 
  anchors = torch.zeros((B, A, H, W, 4),dtype=torch.float32, device=grid.device)
  anc = anc/2
...
def GenerateProposal(anchors, offsets, method='YOLO'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  anchors_temp = torch.zeros_like(anchors)
  
  anchors_temp[..., 0] = (anchors[..., 2] + anchors[..., 0]) / 2
  anchors_temp[..., 1] = (anchors[..., 3] + anchors[..., 1]) / 2
  anchors_temp[..., 2] = anchors[..., 2] - anchors[..., 0]
  anchors_temp[..., 3] = anchors[..., 3] - anchors[..., 1]
  
  proposals_temp = torch.zeros_like(anchors_temp)
  
  proposals_temp[..., 0] = anchors_temp[..., 0] + offsets[..., 0]
  proposals_temp[..., 1] = anchors_temp[..., 1] + offsets[..., 1]
  proposals_temp[..., 2] = anchors_temp[..., 2] * torch.exp(offsets[..., 2])
  proposals_temp[..., 3] = anchors_temp[..., 3] * torch.exp(offsets[..., 3])
  
  proposals = torch.zeros_like(proposals_temp)
  proposals[..., 0] = proposals_temp[..., 0] - proposals_temp[..., 2] / 2
  proposals[..., 1] = proposals_temp[..., 1] - proposals_temp[..., 3] / 2
  proposals[..., 2] = proposals_temp[..., 0] + proposals_temp[..., 2] / 2
  proposals[..., 3] = proposals_temp[..., 1] + proposals_temp[..., 3] / 2
...
def IoU(proposals, bboxes):
  ...
  proposal_length = proposals[..., 2] - proposals[..., 0]
  proposal_width = proposals[..., 3] - proposals[..., 1]
  proposal_area = abs(proposal_length * proposal_width)
  
  bboxes_length = bboxes[..., 2] - bboxes[..., 0]
  bboxes_width = bboxes[..., 3] - bboxes[..., 1]
  bboxes_area = abs(bboxes_length * bboxes_width)
  
  union_length = (torch.min(proposals[..., 2], bboxes[..., 2].unsqueeze(1)) - torch.max(proposals[..., 0], bboxes[..., 0].unsqueeze(1))).clamp(0)
  union_width = (torch.min(proposals[..., 3], bboxes[..., 3].unsqueeze(1)) - torch.max(proposals[..., 1], bboxes[..., 1].unsqueeze(1))).clamp(0)
  union_area = union_length * union_width
  
  iou_mat = union_area / (proposal_area + bboxes_area.unsqueeze(1) - union_area)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return iou_mat
...
class PredictionNetwork(nn.Module):
  ...
  def forward(self, features, pos_anchor_idx=None, neg_anchor_idx=None):
    ...
    print(conf_offsets_package.shape)
    # apply sigmoid fucntion to fx,fy,conf
...
class SingleStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    anc_list = GenerateAnchor(self.anchor_list.cuda(), grid_list)
    # iii) Compute IoU between anchors and GT boxes and then determine activated/#
    #      negative anchors, and GT_conf_scores, GT_offsets, GT_class, 
    ...
    # iv) Compute conf_scores, offsets, class_prob through the prediction network
    conf_scores, offsets, class_prob = self.pred_network(feat,activated_anc_ind, negative_anc_ind)
    # v) Compute the total_loss which is formulated as:                          #
    #    total_loss = w_conf * conf_loss + w_reg * reg_loss + w_cls * cls_loss,  #
    #    where conf_loss is determined by ConfScoreRegression, w_reg by          #
    #    BboxRegression, and w_cls by ObjectClassification. 
